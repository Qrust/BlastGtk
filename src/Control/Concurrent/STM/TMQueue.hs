-- stolen from stm-chan
-- renamed Chan to Queue
-- removed .Compat suffix
-- added Prelude import
-- commented dupTMQueue
...
{-
-- | Duplicate a @TMQueue@: the duplicate queue begins empty, but
-- data written to either queue from then on will be available
-- from both, and closing one copy will close them all. Hence this
-- creates a kind of broadcast queue, where data written by anyone
-- is seen by everyone else.
dupTMQueue :: TMQueue a -> STM (TMQueue a)
dupTMQueue (TMQueue closed queue) = do
    new_queue <- dupTQueue queue
    return (TMQueue closed new_queue)
-}
